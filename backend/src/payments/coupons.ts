...
export async function redeemPromoCode(uid: string, rawCode: string) {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (!promoSnap.exists) throw new Error('Coupon not found')

    const promo = promoSnap.data() as PromoDoc
    if (!promo.active) throw new Error('This coupon is no longer active')
    ...
    if (redemptionSnap.exists) throw new Error('You have already used this coupon')

    tx.update(promoRef, { redemptionCount: FieldValue.increment(1) })
    tx.set(redemptionRef, { uid, redeemedAt: FieldValue.serverTimestamp() })
  })

  const promo = (await promoRef.get()).data() as PromoDoc
  const durationDays = promo.durationDays || 30
  const subscriptionExpiresAt = await applySubscriptionDays(uid, {
    durationDays,
    subscriptionPlan: 'monthly_trial',
    source: 'coupon',
    couponCode: code,
  })
